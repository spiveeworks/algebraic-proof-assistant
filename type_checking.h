...
bool expr_unify_destructive(size_t depth, struct expr a, struct expr b) {
    ...
    while (true) {
        ...
        /* Note, EXPR_GLOBAL and various builtins will provide other ways for
           these things to simplify, maybe based on the number of arguments
           applied or on whether they contain the right patterns. */
        bool a_can_simplify = a.head_type == EXPR_APPLY_LAMBDA;
        bool b_can_simplify = b.head_type == EXPR_APPLY_LAMBDA;

        if (a_intro_count == 0 && a_can_simplify) {
            /* Before we can continue unifying, we need to simplify a, to see
               what its head term is when head-normalised. */
            reduce_expr_once(depth, &a);
            continue;
        } else if (b_intro_count == 0 && b_can_simplify) {
            reduce_expr_once(depth, &b);
            continue;
        } else {
            /* Either a or b has a head term that isn't a lambda or a pi intro,
               so we can stop unifying lambda and pi intros now. If both have
               head terms that aren't lambdo or pi, then unifying intros
               succeeded, and we can try unifying other stuff, otherwise we
